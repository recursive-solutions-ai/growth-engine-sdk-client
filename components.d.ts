...
interface RelatedArticlesProps {
     ...
     * worse than no button (and null is also how the owner's "hide the booking
     * box" switch arrives). When the owner chose their own wording it rides on
     * `cta.buttonLabel` / `cta.lead` and wins over the two props below.
     */
    cta?: BookingCallToAction | null;
    /** Button text when the owner chose none. Defaults to "Book a call". */
    ctaLabel?: string;
    /** One sentence above the button when the owner chose none, e.g. "Not sure what applies to you?" */
...
interface UpdatesFeedProps {
    ...
    locale?: string;
    /**
     * Language code for the content itself ('fr', 'es', …). Items written in that
     * language render translated; the rest fall back to the default language, so
     * a partly translated feed still renders in full. Defaults to `locale` — on a
     * multilingual site the two are almost always the same, and having to pass
     * both is how one of them ends up wrong.
     */
    language?: string;
    translations?: UpdatesFeedTranslations;
}
/**
 * Renders one update list. The body is markdown, rendered with
 ...
 */
declare function UpdatesFeed({ listSlug, limit, className, renderItem, locale, language, translations, }: UpdatesFeedProps): react_jsx_runtime.JSX.Element;
