...
// src/hooks/useContactClickTracking.ts
import { useEffect } from "react";

// src/triggers/onContactClick.ts
async function onContactClick(event) {
  await sdkFetch("/contact-click", {
    method: "POST",
    body: JSON.stringify({
      kind: event.kind,
      href: event.href,
...
// src/hooks/useContactClickTracking.ts
var SESSION_KEY = "rs_contact_session";
function getSessionId() {
  try {
    const existing = window.sessionStorage.getItem(SESSION_KEY);
    if (existing) return existing;
    const id = typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : `s_${Date.now()}_${Math.random().toString(36).slice(2)}`;
...
function classifyContactAnchor(anchor, bookingHosts) {
  const raw = anchor.getAttribute("href") ?? "";
  const href = raw.trim();
  if (anchor.hasAttribute("data-rs-booking")) return "booking";
  if (/^tel:/i.test(href)) return "phone";
  if (/^mailto:/i.test(href)) return "email";
  if (bookingHosts.length > 0 && /^https?:\/\//i.test(href)) {
    try {
      const host = new URL(href).hostname;
      if (bookingHosts.some((pattern) => hostMatches(host, pattern))) return "booking";
    } catch {
      return null;
    }
  }
...
function useContactClickTracking(options = {}) {
  const enabled = options.enabled !== false;
  const bookingHostsKey = (options.bookingHosts ?? []).join(",");
  useEffect(() => {
    if (!enabled) return;
    if (typeof document === "undefined") return;
    const bookingHosts = bookingHostsKey ? bookingHostsKey.split(",").filter(Boolean) : [];
    function handleClick(event) {
      try {
        ...
        if (!anchor) return;
        const kind = classifyContactAnchor(anchor, bookingHosts);
        if (!kind) return;
        const href = (anchor.getAttribute("href") ?? "").trim();
        if (!href && kind !== "booking") return;
        void onContactClick({
          kind,
          href,
        ...
        console.error("[GrowthEngine] contact click tracking failed:", err);
      }
    }
...
function GrowthEngineProvider({
  ...
  useContactClickTracking({ enabled: trackContactClicks, bookingHosts });
  useEffect2(() => {
    let cancelled = false;
...
function usePages(options) {
  return useContent("landing_page", options);
}
...
async function fetchPage(slug, locale) {
  const params = new URLSearchParams({ type: "landing_page", slug });
  if (locale) params.set("locale", locale);
  const res = await sdkFetch(`/content?${params.toString()}`);
  return res.error ? null : res.data;
}
...
import { useState as useState4, useEffect as useEffect6, useMemo } from "react";
...
function useUpdateLists(language) {
  ...
  useEffect7(() => {
    ...
    async function load() {
      setLoading(true);
      const params = new URLSearchParams({ action: "lists" });
      if (language) params.set("lang", language);
      const res = await sdkFetch(`/updates?${params.toString()}`);
      if (cancelled) return;
      if (res.error) {
        setError(res.error);
      } else {
...
function useUpdates(listSlug, options) {
  ...
  const [list, setList] = useState5(null);
  const [loading, setLoading] = useState5(true);
  const [error, setError] = useState5(null);
...
function useVariables() {
  const [variables2, setVariables] = useState6({});
  ...
  useEffect8(() => {
    ...
    async function load() {
      setLoading(true);
      const res = await sdkFetch("/variables");
      if (cancelled) return;
      if (res.error) {
        setError(res.error);
      } else {
...
import { useState as useState8, useEffect as useEffect10, useCallback } from "react";
function useJobStatus(jobId) {
  const [job, setJob] = useState8(null);
...
function buildWebSiteLd(input) {
  const name = input.name?.trim();
  const url = input.url?.trim();
  if (!name || !url) return null;
  return { "@context": CONTEXT, "@type": "WebSite", name, url };
}
...
function renderJsonLdScriptTag(data, pretty = true) {
  return `<script type="application/ld+json">
${serializeJsonLd(data, pretty)}
<\/script>`;
}
...
function BusinessJsonLd({ config, organization }) {
  const ld = organization ? buildOrganizationLdFromConfig(config) : buildLocalBusinessLd(config);
  if (!ld) return null;
  return /* @__PURE__ */ jsx3(JsonLd, { data: ld });
}
...
function MetaTags({ title, description, image, url, type = "article" }) {
  return /* @__PURE__ */ jsxs(Head, { children: [
    title && /* @__PURE__ */ jsx4("meta", { property: "og:title", content: title }),
    description && /* @__PURE__ */ jsx4("meta", { property: "og:description", content: description }),
    image && /* @__PURE__ */ jsx4("meta", { property: "og:image", content: image }),
    url && /* @__PURE__ */ jsx4("meta", { property: "og:url", content: url }),
    /* @__PURE__ */ jsx4("meta", { property: "og:type", content: type }),
    /* @__PURE__ */ jsx4("meta", { name: "twitter:card", content: image ? "summary_large_image" : "summary" }),
    title && /* @__PURE__ */ jsx4("meta", { name: "twitter:title", content: title }),
    description && /* @__PURE__ */ jsx4("meta", { name: "twitter:description", content: description }),
    image && /* @__PURE__ */ jsx4("meta", { name: "twitter:image", content: image })
  ] });
}
...
export {
  ...
  RELATED_ARTICLES_MIN,
  SDK_VERSION,
  buildBlogPostingLd,
  buildBlogPostingNodes,
  buildBlogTopics,
  buildBreadcrumbListLd,
  buildFaqPageLd,
  buildFormSchema,
  buildLocalBusinessLd,
  buildOrganizationLdFromConfig,
  buildWebSiteLd,
  classifyContactAnchor,
  extractFaqPairs,
  fetchAuthor,
  fetchAuthorPosts,
  fetchBlog,
