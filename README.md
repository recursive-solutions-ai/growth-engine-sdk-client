...
| Export | Type | Description |
|--------|------|-------------|
| `useUpdateLists()` | Hook | List every update feed. Returns `{ lists: UpdateListSummary[], loading, error }` |
| `useUpdates(listSlug, { limit? })` | Hook | Published items of one list, newest first. Returns `{ items: UpdateFeedItem[], list: UpdateListSummary \| null, loading, error }` |
| `<UpdatesFeed listSlug limit? className? renderItem? locale? translations? />` | Component (`/components`) | Renders a list. Markdown bodies are rendered with `marked`; pass `renderItem` to take over rendering |

Dates (`eventDate`, `publishedAt`) arrive as ISO strings. Only published items are ever returned.

### Variables

| Export | Type | Description |
|--------|------|-------------|
| `useVariables()` | Hook | Tenant-wide key/value map. Returns `{ variables: Record<string, string>, loading, error }` |

Company-scope values only — per-user variables are never exposed to the public site.

### Jobs & Triggers

| Export | Type | Description |
|--------|------|-------------|
| `useJobStatus(jobId)` | Hook | Poll job status. Returns `{ job, loading, error, refresh }`; `job` is a summary `{ jobId, type, status, createdAt }`, and only `blog.generate` / `social.sync` jobs are visible |

Jobs cannot be started from the browser. `triggerBlogGen` / `triggerSocialSync` were removed from this package: they posted to the public `/api/rs/jobs` route, which let any site visitor spend the tenant's AI budget. Enqueue from server code instead, with `enqueueJob`, `triggerBlogGen` or `triggerSocialSync` from `@growth-engine/sdk-server` (behind your own auth).
