...
function BlogCard({ slug, title, content, heroImageUrl, seoDesc, createdAt, locale, localePrefix, author }) {
  const date = formatDate(createdAt, locale);
  const isoDate2 = new Date(createdAt).toISOString();
  const preview = seoDesc ?? content.replace(/<[^>]*>/g, "").slice(0, 160) + "...";
  const prefix = localePrefix ?? `/${locale}`;
  return /* @__PURE__ */ jsx("article", { itemScope: true, itemType: "https://schema.org/BlogPosting", children: /* @__PURE__ */ jsxs(Link, { href: `${prefix}/blog/${slug}`, className: "card bg-base-100 shadow-sm border border-base-200 hover:shadow-md transition-shadow group", children: [
    ...
    /* @__PURE__ */ jsxs("div", { className: "card-body", children: [
      /* @__PURE__ */ jsx("time", { className: "text-xs text-base-content/50", dateTime: isoDate2, children: date }),
      author && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-1.5 text-xs text-base-content/60", children: [
        author.avatarUrl ? /* @__PURE__ */ jsx(
          "img",
          {
        ...
        ) : /* @__PURE__ */ jsx("span", { className: "w-5 h-5 rounded-full bg-base-300 text-base-content/70 flex items-center justify-center text-[10px] font-semibold", children: author.name.charAt(0).toUpperCase() }),
...
// ../types/src/json-ld.ts
function normalizeLdKeywords(value) {
  let list = value;
  if (typeof value === "string") {
    ...
    else if (!Array.isArray(list)) return [];
  }
...
var CJK_CHAR = /[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f\uac00-\ud7af]/;
function countLdWords(text2) {
  let cjk = 0;
  let plain = "";
  ...
  const latin = plain.trim() === "" ? 0 : plain.trim().split(/\s+/).length;
  return cjk + latin;
}
function stripMarkdown(md) {
  return md.replace(/```[\s\S]*?```/g, " ").replace(/`([^`]*)`/g, "$1").replace(/!\[[^\]]*\]\([^)]*\)/g, " ").replace(/\[([^\]]*)\]\([^)]*\)/g, "$1").replace(/^\s{0,3}>\s?/gm, "").replace(/^\s{0,3}[-*+]\s+/gm, "").replace(/^\s{0,3}\d+\.\s+/gm, "").replace(/\*\*([^*]+)\*\*/g, "$1").replace(/\*([^*]+)\*/g, "$1").replace(/__([^_]+)__/g, "$1").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
}
var LD_JSON_SCRIPT_RAW = /<script\b[^>]*application\/ld\+json[^>]*>[\s\S]*?<\/script\s*>/gi;
var LD_JSON_SCRIPT_ESCAPED = /&lt;script\b(?:(?!&gt;)[\s\S])*?application\/ld\+json(?:(?!&gt;)[\s\S])*?&gt;[\s\S]*?&lt;\/script\s*&gt;/gi;
var FENCE_LINE = /^\s{0,3}(```|~~~)/;
function stripJsonLdScriptBlocks(markdown) {
  if (!markdown) return { content: markdown ?? "", removed: 0 };
  const segments = [];
  let inFence = false;
  for (const line of markdown.split("\n")) {
    const isFenceMarker = FENCE_LINE.test(line);
    const fenced = inFence || isFenceMarker;
  ...
  const rebuilt = segments.map((segment) => {
    const text2 = segment.lines.join("\n");
    if (segment.fenced) return text2;
    return text2.replace(LD_JSON_SCRIPT_RAW, count).replace(LD_JSON_SCRIPT_ESCAPED, count);
  }).join("\n");
  if (removed === 0) return { content: markdown, removed: 0 };
  return { content: rebuilt.replace(/\n{3,}/g, "\n\n").trimEnd(), removed };
}
var INTERROGATIVE_OPENERS = /^(how|what|why|when|where|who|whom|whose|which|can|could|should|shall|do|does|did|is|are|was|were|will|would|may|might|must|am)\b/i;
var MIN_ANSWER_CHARS = 40;
var MAX_ANSWER_CHARS = 1200;
var MIN_FAQ_PAIRS = 2;
var MAX_FAQ_PAIRS = 20;
function isQuestionHeading(text2) {
  const t = text2.trim();
  if (t.length < 8 || t.length > 200) return false;
  if (t.endsWith("?") || t.endsWith("\uFF1F")) return true;
  return INTERROGATIVE_OPENERS.test(t);
}
function extractFaqPairs(markdown) {
  if (!markdown) return [];
  const { content } = stripJsonLdScriptBlocks(markdown);
  ...
  const flush = () => {
    if (!current) return;
    const answer = stripMarkdown(current.body.join("\n"));
    if (answer.length >= MIN_ANSWER_CHARS) {
      pairs.push({
        question: current.question,
        answer: answer.length > MAX_ANSWER_CHARS ? `${answer.slice(0, MAX_ANSWER_CHARS).replace(/\s+\S*$/, "")}\u2026` : answer
      });
    }
    current = null;
  };
  for (const line of lines) {
    ...
    const heading = /^\s{0,3}(#{2,4})\s+(.+?)\s*#*\s*$/.exec(line);
    if (heading) {
      const level = heading[1].length;
      const text2 = stripMarkdown(heading[2]);
      if (current && level <= current.level) flush();
      if (isQuestionHeading(text2)) {
        current = { question: text2, level, body: [] };
      }
      continue;
    }
    if (current) current.body.push(line);
  ...
  flush();
  return pairs.length >= MIN_FAQ_PAIRS ? pairs.slice(0, MAX_FAQ_PAIRS) : [];
}
var CONTEXT = "https://schema.org";
function buildPostalAddressLd(address) {
  if (!address) return null;
  if (typeof address === "string") {
    const trimmed = address.trim();
    if (!trimmed) return null;
    return { "@type": "PostalAddress", streetAddress: trimmed };
  }
  const street = [address.street, address.street2].filter((p) => p && p.trim()).join(", ");
  const node = omitEmpty({
    "@type": "PostalAddress",
    streetAddress: street || address.streetAddress || null,
    addressLocality: address.city ?? address.addressLocality ?? null,
    addressRegion: address.region ?? address.addressRegion ?? null,
    postalCode: address.postalCode ?? null,
    addressCountry: address.country ?? address.addressCountry ?? null
  });
  return Object.keys(node).length > 1 ? node : null;
}
function buildOrganizationLd(business) {
  const name = business.name?.trim();
  if (!name) return null;
  const sameAs = (business.sameAs ?? []).filter((u) => typeof u === "string" && u.trim() !== "").map((u) => u.trim());
  const node = {
    "@context": CONTEXT,
    "@type": business.localBusiness ? "LocalBusiness" : "Organization",
    ...omitEmpty({
      name,
      ...
      sameAs: sameAs.length > 0 ? Array.from(new Set(sameAs)) : null
    })
  };
  const address = buildPostalAddressLd(business.address);
  if (address) node.address = address;
  return node;
}
function buildLocalBusinessLd(business) {
  return buildOrganizationLd({ ...business, localBusiness: true });
}
function buildArticleLd(article) {
  const node = {
    "@context": CONTEXT,
    ...
    ...omitEmpty({
      ...
      dateModified: isoDate(article.dateModified) ?? isoDate(article.datePublished),
      inLanguage: article.inLanguage ?? null,
      keywords: normalizeLdKeywords(article.keywords).join(", ") || null
    })
  };
  if (article.url) {
    node.mainEntityOfPage = { "@type": "WebPage", "@id": article.url };
  }
  if (article.body) {
    const words = countLdWords(stripMarkdown(article.body));
    if (words > 0) node.wordCount = words;
  }
  if (article.author?.name) {
    node.author = omitEmpty({
      "@type": "Person",
  ...
  if (article.publisher?.name) {
    node.publisher = omitEmpty({
      "@type": "Organization",
      name: article.publisher.name.trim(),
      url: article.publisher.url ?? null,
      logo: article.publisher.logo ?? null
    });
  }
  return node;
}
function buildFaqPageLd(pairs) {
  const usable = pairs.filter((p) => p.question.trim() && p.answer.trim());
  if (usable.length < MIN_FAQ_PAIRS) return null;
  return {
    "@context": CONTEXT,
    "@type": "FAQPage",
    mainEntity: usable.slice(0, MAX_FAQ_PAIRS).map((p) => ({
      "@type": "Question",
...
function buildBreadcrumbListLd(items) {
  const usable = items.filter((i) => i.name.trim());
  if (usable.length < 2) return null;
  return {
    "@context": CONTEXT,
    "@type": "BreadcrumbList",
    itemListElement: usable.map((item, index2) => ({
      "@type": "ListItem",
      position: index2 + 1,
      name: item.name.trim(),
...
function buildBlogPostingGraph(input) {
  const nodes = [
    buildArticleLd({ ...input, body: input.body ?? input.content ?? null })
  ];
  const pairs = input.faqPairs ?? (input.content ? extractFaqPairs(input.content) : []);
  const faq = buildFaqPageLd(pairs);
  if (faq) nodes.push(faq);
  const crumbs = buildBreadcrumbListLd(input.breadcrumbs ?? []);
  if (crumbs) nodes.push(crumbs);
  return nodes;
}
function blogBreadcrumbs(options) {
  const { siteName, siteUrl, postTitle, postUrl } = options;
  if (!siteUrl || !postTitle) return [];
  let origin;
  let blogUrl;
  try {
    ...
    if (postUrl) {
      ...
      segments.pop();
      blogUrl = `${p.origin}/${segments.join("/")}`.replace(/\/+$/, "") || p.origin;
    } else {
      blogUrl = `${origin}/blog`;
    }
  } catch {
  ...
  if (blogUrl === origin) {
    return [
      { name: siteName?.trim() || "Home", url: origin },
      { name: postTitle.trim(), ...postUrl ? { url: postUrl } : {} }
    ];
  }
  return [
    { name: siteName?.trim() || "Home", url: origin },
    { name: options.blogLabel ?? "Blog", url: blogUrl },
    { name: postTitle.trim(), ...postUrl ? { url: postUrl } : {} }
  ];
}
function serializeJsonLd(data, pretty = false) {
  const payload = Array.isArray(data) && data.length === 1 ? data[0] : data;
  return JSON.stringify(payload, null, pretty ? 2 : 0).replace(/</g, "\\u003c").replace(/>/g, "\\u003e").replace(/&/g, "\\u0026").replace(/\u2028/g, "\\u2028").replace(/\u2029/g, "\\u2029");
}

// src/seo/json-ld.ts
function toBusinessInput(config) {
  return {
    name: config.name,
    ...
    email: config.email,
    // The Turso column already stores schema.org-shaped keys; the builder
    // accepts both that and the platform's typed PostalAddress parts.
    address: config.address ?? null
  };
}
function buildBlogPostingNodes(post, author, business, options) {
  const siteUrl = options?.siteUrl ?? business?.website ?? null;
  const breadcrumbs = blogBreadcrumbs({
    siteName: options?.siteName ?? business?.name ?? null,
    siteUrl,
    postTitle: post.title,
    postUrl: options?.url ?? null
  });
  return buildBlogPostingGraph({
    ...
    breadcrumbs,
    ...author ? { author: { name: author.name, url: author.websiteUrl, image: author.avatarUrl } } : {},
    ...business ? { publisher: toBusinessInput(business) } : {}
  });
}
function buildLocalBusinessLd2(config) {
  return buildOrganizationLd({ ...toBusinessInput(config), localBusiness: true });
}
function buildOrganizationLdFromConfig(config) {
  return buildOrganizationLd(toBusinessInput(config));
}
...
function BlogContent({
  ...
  const nodes = post && !disableJsonLd ? buildBlogPostingNodes(post, author, business, {
    ...canonicalUrl ? { url: canonicalUrl } : {},
    ...siteUrl !== void 0 ? { siteUrl } : {},
    ...siteName !== void 0 ? { siteName } : {}
  }) : [];
  const jsonLd = articleLdOnly ? nodes.slice(0, 1) : nodes;
  const body = stripJsonLdScriptBlocks(html).content;
  return /* @__PURE__ */ jsxs3("article", { itemScope: true, itemType: "https://schema.org/BlogPosting", children: [
    jsonLd.length > 0 && /* @__PURE__ */ jsx4(JsonLd, { data: jsonLd }),
...
function BlogList({
  ...
  const perPage = postsPerPage ?? BLOG_POSTS_PER_PAGE;
  const [search, setSearch] = useState("");
  const [clientPage, setClientPage] = useState(1);
  const searching = search.trim().length > 0;
  const linkMode = paginationBasePath !== void 0 && !searching;
  const currentPage = linkMode ? Math.max(1, page ?? 1) : clientPage;
  ...
  const pageHref = (p) => p <= 1 ? paginationBasePath : `${paginationBasePath}/page/${p}`;
  return /* @__PURE__ */ jsxs5("section", { "aria-label": "Blog posts", children: [
    ...
    ] }) : /* @__PURE__ */ jsx6("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6", children: paginated.map((post) => {
      const author = authorMap && post.authorId ? authorMap.get(post.authorId) ?? null : null;
      const cardAuthor = author ? { name: author.name, slug: author.slug, avatarUrl: author.avatarUrl } : null;
      return /* @__PURE__ */ jsx6(
        BlogCard,
        {
          ...post,
          locale,
          ...localePrefix !== void 0 ? { localePrefix } : {},
          author: cardAuthor
        },
        post.slug
      );
    }) }),
    totalPages > 1 && /* @__PURE__ */ jsx6("div", { className: "flex justify-center mt-8", children: /* @__PURE__ */ jsx6("nav", { "aria-label": "Blog pagination", className: "join", children: linkMode ? /* @__PURE__ */ jsxs5(Fragment, { children: [
      ...
      ) : /* @__PURE__ */ jsx6("span", { className: "join-item btn btn-sm btn-disabled", "aria-disabled": "true", children: "\xAB" }),
      Array.from({ length: totalPages }, (_, i) => i + 1).map(
        (p) => p === currentPage ? /* @__PURE__ */ jsx6(
          "span",
          {
      ...
      ) : /* @__PURE__ */ jsx6("span", { className: "join-item btn btn-sm btn-disabled", "aria-disabled": "true", children: "\xBB" })
      ...
      Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => /* @__PURE__ */ jsx6(
        "button",
        {
          className: `join-item btn btn-sm ${p === clientPage ? "btn-active" : ""}`,
          onClick: () => setClientPage(p),
          "aria-label": `Page ${p}`,
          "aria-current": p === clientPage ? "page" : void 0,
          children: p
        },
        p
      )),
...
function RelatedPosts({ posts, currentSlug, locale, localePrefix, heading }) {
...
function RelatedArticles({
  ...
  if (posts.length === 0 && !cta) return null;
  const columns = posts.length >= 4 ? "lg:grid-cols-4" : "md:grid-cols-3";
  const label = cta?.buttonLabel || ctaLabel || BOOKING_CTA_DEFAULT_LABEL;
  const description = cta?.lead || ctaDescription;
  return /* @__PURE__ */ jsxs7("aside", { "aria-label": heading, className: "mt-16", children: [
    posts.length > 0 && /* @__PURE__ */ jsxs7(Fragment2, { children: [
      /* @__PURE__ */ jsx8("h2", { className: "text-2xl font-bold mb-6", children: heading }),
      /* @__PURE__ */ jsx8("div", { className: `grid grid-cols-1 sm:grid-cols-2 ${columns} gap-6`, children: posts.map((post) => /* @__PURE__ */ jsx8(
        BlogCard,
        {
          ...post,
          locale,
    ...
    cta && /* @__PURE__ */ jsxs7("div", { className: "mt-10 rounded-xl border border-base-200 bg-base-100 p-6 sm:p-8 text-center shadow-sm", children: [
      description && /* @__PURE__ */ jsx8("p", { className: "text-base-content/70 mb-4", children: description }),
      cta.href.startsWith("/") ? /* @__PURE__ */ jsx8(Link3, { href: cta.href, className: "btn btn-primary btn-lg", children: label }) : /* @__PURE__ */ jsx8("a", { href: cta.href, className: "btn btn-primary btn-lg", children: label })
    ] })
  ] });
}

// src/components/blog/TopicChips.tsx
...
function TopicChips({
  ...
  const prefix = localePrefix ?? `/${locale}`;
  return /* @__PURE__ */ jsxs8("nav", { "aria-label": label, className: className ?? "mb-6", children: [
    /* @__PURE__ */ jsx9("p", { className: "text-xs uppercase tracking-wider text-base-content/50 mb-2", children: label }),
    /* @__PURE__ */ jsx9("div", { className: "flex flex-wrap gap-2", children: topics.map((topic) => {
      const text2 = showCounts ? `${topic.label} (${topic.postCount})` : topic.label;
      if (topic.slug === currentSlug) {
        return /* @__PURE__ */ jsx9("span", { className: "badge badge-primary badge-lg", "aria-current": "page", children: text2 }, topic.slug);
      }
      return /* @__PURE__ */ jsx9(
        Link4,
        {
          href: `${prefix}${BLOG_TOPIC_PATH}/${encodeURIComponent(topic.slug)}`,
...
function addressFromConfig(business) {
  if (!business) return null;
  const raw = business.address;
  if (!raw) return null;
  if (typeof raw === "string") return raw.trim() || null;
  const parts = {
    street: typeof raw.street === "string" ? raw.street : void 0,
    city: typeof raw.city === "string" ? raw.city : void 0,
    region: typeof raw.region === "string" ? raw.region : void 0,
    postalCode: typeof raw.postalCode === "string" ? raw.postalCode : void 0,
    country: typeof raw.country === "string" ? raw.country : void 0
  };
...
function LandingPageContent({
  ...
  const nodes = [];
  if (!disableJsonLd) {
    const address = addressFromConfig(business);
    const localBusiness = buildLocalBusinessLd({
      name: business?.name ?? null,
      description: business?.description ?? null,
      url: siteUrl ?? business?.website ?? null,
      telephone: business?.phone ?? null,
      email: business?.email ?? null,
      address,
      localBusiness: Boolean(address)
    });
    if (localBusiness) nodes.push(localBusiness);
    const faq = buildFaqPageLd(extractFaqPairs(page.content));
    if (faq) nodes.push(faq);
    const home = siteUrl ?? business?.website ?? null;
    const breadcrumb = buildBreadcrumbListLd([
      { name: siteName ?? business?.name ?? "Home", ...home ? { url: home } : {} },
      { name: page.title, ...canonicalUrl ? { url: canonicalUrl } : {} }
    ]);
    if (breadcrumb) nodes.push(breadcrumb);
  }
  const body = stripJsonLdScriptBlocks(page.content).content;
  return /* @__PURE__ */ jsxs9("article", { children: [
    nodes.length > 0 && /* @__PURE__ */ jsx10(JsonLd, { data: nodes }),
    !disableMeta && /* @__PURE__ */ jsx10(
      MetaTags,
      {
    ...
    /* @__PURE__ */ jsx10(
      ...
      {
        className: "prose prose-lg max-w-none",
        dangerouslySetInnerHTML: { __html: marked2.parse(body, { async: false }) }
      }
    )
  ] });
}
...
function BusinessJsonLd({ config, organization }) {
  const ld = organization ? buildOrganizationLdFromConfig(config) : buildLocalBusinessLd2(config);
  if (!ld) return null;
  return /* @__PURE__ */ jsx11(JsonLd, { data: ld });
}

// src/components/forms/FormCard.tsx
import Link5 from "next/link";
import { jsx as jsx12, jsxs as jsxs10 } from "react/jsx-runtime";
function FormCard({ slug, name, description, locale, localePrefix }) {
  const prefix = localePrefix ?? `/${locale}`;
  return /* @__PURE__ */ jsx12(
    Link5,
    {
      href: `${prefix}/forms/${slug}`,
      className: "card bg-base-100 border border-base-200 shadow-sm hover:shadow-md transition-shadow",
      children: /* @__PURE__ */ jsxs10("div", { className: "card-body", children: [
        /* @__PURE__ */ jsx12("h2", { className: "card-title", children: name }),
        description && /* @__PURE__ */ jsx12("p", { className: "text-base-content/60", children: description })
      ] })
    }
  );
}
...
var DEFAULT_NEWSLETTER_OPT_IN_LABEL = "Email me your newsletter";
...
// ../../node_modules/.pnpm/drizzle-orm@0.38.4_@libsql+client@0.15.15_@opentelemetry+api@1.9.0_@types+react@19.2.14_react@19.2.4/node_modules/drizzle-orm/sqlite-core/indexes.js
...
// ../../node_modules/.pnpm/drizzle-orm@0.38.4_@libsql+client@0.15.15_@opentelemetry+api@1.9.0_@types+react@19.2.14_react@19.2.4/node_modules/drizzle-orm/sqlite-core/primary-keys.js
...
async function submitForm(slug, data) {
  ...
  const sourcePath = typeof window !== "undefined" ? window.location.pathname : void 0;
  const res = await sdkFetch(
    `/forms?action=submit&slug=${encodeURIComponent(slug)}`,
    {
      method: "POST",
      body: JSON.stringify({ data, ...sourcePath ? { sourcePath } : {} })
    }
  );
  if (res.error) {
...
function FormRenderer({ form, translations, onSubmitSuccess }) {
  ...
  const formSettings = readSettings(form.settings);
  const showNewsletterOptIn = formSettings?.kind !== "newsletter" && formSettings?.newsletterOptIn?.enabled === true;
  useEffect2(() => {
    ...
    if (showNewsletterOptIn) initial[NEWSLETTER_OPT_IN_FIELD] = false;
    setFormData(initial);
    setInitialized(true);
  }, [form.fields, initialized, showNewsletterOptIn]);
  ...
  if (submitted) {
    const settings2 = formSettings;
    return /* @__PURE__ */ jsxs11("div", { className: "mx-auto max-w-lg rounded-xl border bg-base-100 p-8 text-center shadow-lg", children: [
      /* @__PURE__ */ jsx13("div", { className: "mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-success/20", children: /* @__PURE__ */ jsx13("svg", { className: "h-6 w-6 text-success", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: /* @__PURE__ */ jsx13("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M5 13l4 4L19 7" }) }) }),
      /* @__PURE__ */ jsx13("h2", { className: "text-xl font-semibold", children: settings2?.successMessage ?? translations?.defaultSuccessMessage ?? "Thank you for your submission!" })
    ] });
  }
  const sortedFields = [...form.fields].sort((a, b) => a.order - b.order);
  const settings = formSettings;
  return /* @__PURE__ */ jsxs11("div", { className: "mx-auto max-w-lg", children: [
    /* @__PURE__ */ jsx13("h1", { className: "text-3xl font-bold", children: form.name }),
    form.description && /* @__PURE__ */ jsx13("p", { className: "mt-2 text-base-content/60", children: form.description }),
    /* @__PURE__ */ jsxs11("form", { onSubmit: handleSubmit, className: "mt-8 space-y-4", children: [
      sortedFields.map((field) => /* @__PURE__ */ jsxs11("div", { className: "form-control w-full", children: [
        /* @__PURE__ */ jsx13("label", { htmlFor: field.name, className: "label", children: /* @__PURE__ */ jsxs11("span", { className: "label-text", children: [
          field.label,
          field.required && /* @__PURE__ */ jsx13("span", { className: "ml-1 text-error", children: "*" })
        ] }) }),
        field.type === "textarea" ? /* @__PURE__ */ jsx13(
        ...
        ) : field.type === "select" ? /* @__PURE__ */ jsxs11(
          "select",
          {
            id: field.name,
            required: field.required,
        ...
        ) : field.type === "checkbox" ? /* @__PURE__ */ jsxs11("label", { className: "label cursor-pointer justify-start gap-3", children: [
          /* @__PURE__ */ jsx13(
            "input",
            {
              type: "checkbox",
          ...
          /* @__PURE__ */ jsx13("span", { className: "label-text", children: field.placeholder })
        ] }) : /* @__PURE__ */ jsx13(
          "input",
          {
            type: field.type,
      ...
      showNewsletterOptIn && /* @__PURE__ */ jsx13("div", { className: "form-control w-full", children: /* @__PURE__ */ jsxs11("label", { className: "label cursor-pointer justify-start gap-3", children: [
        /* @__PURE__ */ jsx13(
          "input",
          {
            type: "checkbox",
            id: NEWSLETTER_OPT_IN_FIELD,
            checked: Boolean(formData[NEWSLETTER_OPT_IN_FIELD]),
            onChange: (e) => updateField(NEWSLETTER_OPT_IN_FIELD, e.target.checked),
            className: "checkbox"
          }
        ),
        /* @__PURE__ */ jsx13("span", { className: "label-text", children: formSettings?.newsletterOptIn?.label ?? DEFAULT_NEWSLETTER_OPT_IN_LABEL })
      ] }) }),
      error && /* @__PURE__ */ jsx13("div", { className: "alert alert-error", children: /* @__PURE__ */ jsx13("span", { children: error }) }),
      /* @__PURE__ */ jsx13(
        "button",
        {
          type: "submit",
...
async function subscribeToNewsletter(data) {
  ...
  return res.error ? { ok: false, error: res.error } : { ok: res.data.ok };
}
async function unsubscribeFromNewsletter(data) {
  const res = await sdkFetch(
    "/newsletter?action=unsubscribe",
    {
      method: "POST",
      body: JSON.stringify(data)
    }
...
function NewsletterSignup({
  ...
  const [subscribed, setSubscribed] = useState4(false);
  ...
  if (subscribed) {
    return /* @__PURE__ */ jsxs12("div", { className: `rounded-xl border bg-base-100 p-6 text-center ${className ?? ""}`.trim(), children: [
      /* @__PURE__ */ jsx14("div", { className: "mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-success/20", children: /* @__PURE__ */ jsx14("svg", { className: "h-5 w-5 text-success", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: /* @__PURE__ */ jsx14("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M5 13l4 4L19 7" }) }) }),
      /* @__PURE__ */ jsx14("p", { className: "font-semibold", children: successMessage })
    ] });
  }
  return /* @__PURE__ */ jsxs12("div", { className: `rounded-xl border bg-base-100 p-6 ${className ?? ""}`.trim(), children: [
    title && /* @__PURE__ */ jsx14("h2", { className: "text-xl font-semibold", children: title }),
    description && /* @__PURE__ */ jsx14("p", { className: "mt-1 text-base-content/60", children: description }),
    /* @__PURE__ */ jsxs12("form", { onSubmit: handleSubmit, className: "mt-4 space-y-3", children: [
      collectName && /* @__PURE__ */ jsx14(
        "input",
        {
          type: "text",
          value: name,
      ...
      error && /* @__PURE__ */ jsx14("p", { className: "text-sm text-error", children: error }),
      /* @__PURE__ */ jsx14("button", { type: "submit", disabled: submitting, className: "btn btn-primary w-full", children: submitting ? "Subscribing..." : buttonLabel })
    ] })
  ] });
}

// src/components/newsletter/NewsletterUnsubscribe.tsx
import { useState as useState5 } from "react";
import { jsx as jsx15, jsxs as jsxs13 } from "react/jsx-runtime";
function NewsletterUnsubscribe({
  title = "Unsubscribe",
  description = "Enter the email address you want removed from our newsletter.",
  buttonLabel = "Unsubscribe",
  placeholder = "you@example.com",
  successMessage = "If that address was on our list, it has been removed.",
  successHint,
  className,
  onSuccess
}) {
  ...
  if (done) {
    return /* @__PURE__ */ jsxs13("div", { className: `rounded-xl border bg-base-100 p-6 text-center ${className ?? ""}`.trim(), children: [
      /* @__PURE__ */ jsx15("div", { className: "mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-success/20", children: /* @__PURE__ */ jsx15("svg", { className: "h-5 w-5 text-success", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: /* @__PURE__ */ jsx15("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M5 13l4 4L19 7" }) }) }),
      /* @__PURE__ */ jsx15("p", { className: "font-semibold", children: successMessage }),
      successHint && /* @__PURE__ */ jsx15("p", { className: "mt-1 text-sm text-base-content/60", children: successHint })
    ] });
  }
  return /* @__PURE__ */ jsxs13("div", { className: `rounded-xl border bg-base-100 p-6 ${className ?? ""}`.trim(), children: [
    title && /* @__PURE__ */ jsx15("h2", { className: "text-xl font-semibold", children: title }),
    description && /* @__PURE__ */ jsx15("p", { className: "mt-1 text-base-content/60", children: description }),
    /* @__PURE__ */ jsxs13("form", { onSubmit: handleSubmit, className: "mt-4 space-y-3", children: [
      /* @__PURE__ */ jsx15(
        "input",
        {
          type: "email",
      ...
      error && /* @__PURE__ */ jsx15("p", { className: "text-sm text-error", children: error }),
      /* @__PURE__ */ jsx15("button", { type: "submit", disabled: submitting, className: "btn btn-primary w-full", children: submitting ? "Removing..." : buttonLabel })
    ] })
  ] });
}

// src/components/newsletter/NewsletterContent.tsx
import { marked as marked3 } from "marked";
import { jsx as jsx16, jsxs as jsxs14 } from "react/jsx-runtime";
function NewsletterContent({ issue, date }) {
  const displayDate2 = date ?? (issue.sentAt ? new Date(issue.sentAt).toLocaleDateString() : null);
  return /* @__PURE__ */ jsxs14("article", { children: [
    /* @__PURE__ */ jsx16("h1", { className: "text-4xl font-bold mb-2", children: issue.subject }),
    displayDate2 && /* @__PURE__ */ jsx16("time", { className: "text-sm text-base-content/50", children: displayDate2 }),
    /* @__PURE__ */ jsx16(
      "div",
      {
        className: "prose prose-lg max-w-none mt-6",
        dangerouslySetInnerHTML: {
          __html: marked3.parse(issue.content, { async: false })
        }
      }
    )
  ] });
...
// src/components/updates/UpdatesFeed.tsx
...
function useUpdates(listSlug, options) {
  ...
  useEffect3(() => {
    ...
    async function load() {
      setLoading(true);
      const params = new URLSearchParams({ action: "items", list: listSlug });
  ...
  }, [listSlug, limit, language]);
  return { items, list, loading, error };
}
...
function formatDate2(value, locale) {
...
function UpdatesFeed({
  ...
  const contentLanguage = language ?? locale;
  const { items, list, loading, error } = useUpdates(listSlug, {
    ...limit !== void 0 ? { limit } : {},
    ...contentLanguage !== void 0 ? { language: contentLanguage } : {}
  });
  const t = translations ?? DEFAULT_TRANSLATIONS;
  if (error) {
    return /* @__PURE__ */ jsx17("section", { className, "aria-label": list?.name ?? "Updates", children: /* @__PURE__ */ jsx17("p", { className: "text-error", role: "alert", children: error }) });
  }
  return /* @__PURE__ */ jsxs15("section", { className, "aria-label": list?.name ?? "Updates", "aria-busy": loading, children: [
    loading && /* @__PURE__ */ jsx17("p", { className: "text-base-content/50", children: t.loadingLabel }),
    !loading && items.length === 0 && /* @__PURE__ */ jsx17("p", { className: "text-base-content/50", children: t.emptyMessage }),
    items.length > 0 && /* @__PURE__ */ jsx17("ul", { className: "flex flex-col gap-8 list-none p-0 m-0", children: items.map((item) => /* @__PURE__ */ jsx17("li", { children: renderItem ? renderItem(item) : /* @__PURE__ */ jsx17(UpdateItemCard, { item, locale }) }, item.id)) })
  ] });
}
function UpdateItemCard({ item, locale }) {
  const date = displayDate(item);
  return /* @__PURE__ */ jsxs15("article", { children: [
    item.imageUrl && /* @__PURE__ */ jsx17("figure", { className: "aspect-video overflow-hidden mb-3", children: /* @__PURE__ */ jsx17("img", { src: item.imageUrl, alt: item.title, className: "w-full h-full object-cover" }) }),
    /* @__PURE__ */ jsx17("h3", { className: "text-xl font-semibold", children: item.title }),
    date && /* @__PURE__ */ jsx17("time", { className: "text-sm text-base-content/50", dateTime: new Date(date).toISOString(), children: formatDate2(date, locale) }),
    item.body && /* @__PURE__ */ jsx17(
      "div",
      {
        className: "prose max-w-none mt-2",
        dangerouslySetInnerHTML: {
          __html: marked4.parse(item.body, { async: false })
        }
      }
    )
  ] });
